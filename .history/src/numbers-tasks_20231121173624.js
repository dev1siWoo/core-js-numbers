...
function toExponential(number, fractionDigits) {
  ...
  // Get exponent before taking absolute value
  const exp = Math.floor(Math.log10(Math.abs(number)));  

  // Now get absolute value
  let absNumber = Math.abs(number);

  // Scale number between 1-10
  absNumber /= 10 ** exp;

  // Round scaled number
  absNumber = absNumber.toFixed(fractionDigits);

  // Add exponent sign
  const expSign = exp >= 0 ? '+' : '';

  // Prepend negative if original number was negative
  return `${number < 0 ? '-' : ''}${absNumber}e${expSign}${exp}`;
